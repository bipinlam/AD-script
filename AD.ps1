...
function Copy_group($FirstID, $SecondID){
    ...
    $ask = Read-Host "Would you like to copy AD of $SecondID to $FirstID (Y/N) " 
...
$defaul_groups = @("CAU - M365-LIC (E5 Suite)","CAU Distlist - Chatswood Office","CAU - intune - shoreside users")
...
$ask1 = 'y'
 while($ask1 -eq 'y' -or $ask1 -eq 'Y'){
        ...
        try{       
                ...
                $userName= Read-Host " Enter Existing UserID to copy access from "
                ...
                $excel = Read-Host "Would you like to create User Excel File in G:\ (Y/N) ?"
